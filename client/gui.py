#qqqqjfjfjfufujgjbjfgknfknnlkdnjldfjnbjkfbjkdf
from typing import List
from wrappers import Alias, BCPointer, Index, Pubkey, Nym
import config as cfg
import params as pm
from PyQt6.QtWidgets import QApplication, QWidget, QVBoxLayout, QScrollArea, QGroupBox, QGridLayout, QLabel, QSizePolicy, QPushButton, QHBoxLayout, QLineEdit
...
class SocialMediaFeed(QWidget):
    ...
    def initUI(self):
        ...
        scroll_area.setStyleSheet('QScrollArea { background-color: #000; padding: 0; }')

        self.feed_widget = QWidget()

        # Create a vertical layout for the feed widget
        self.feed_layout = QVBoxLayout(self.feed_widget)
        self.feed_layout.setContentsMargins(10, 10, 10, 10) # Set a constant margin between the message boxes and the
    ...
    def show_posts(self,box):
        ...
        for bc in broadcasts:
            ...
            message_box.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding) # Expand the size of the message boxes
            ...
            if bc.parent != BCPointer(b'\x00'*pm.PARENT_LENGTH):
                for p in broadcasts:
                    if p.pointer == bc.parent:
                        ...
                        parent_text1.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Maximum)
                        perent_text2 = QLabel()
                        perent_text2.setText(parent_message)
                        perent_text2.setWordWrap(True)
                        perent_text2.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Maximum)
            ...
            username_label.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Maximum)
            ...
            if bc.parent != BCPointer(b'\x00' * pm.PARENT_LENGTH):
                message_box_layout.addWidget(parent_box, 0, 0)
    ...
    def resizeEvent(self, event):
        for message_box in self.findChildren(QGroupBox):
            ...
            message_text.setMaximumWidth(message_box_width - 20) #type: ignore
            message_text.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding) #type: ignore
